...
CONSTRUCT {
  <https://stat.stadt-zuerich.ch/dataset/WRT-RAUM-ZEIT-BIL-BTA/slice> a qb:Slice ;
...
} WHERE {
  {
    GRAPH <https://linked.opendata.swiss/graph/zh/statistics> {
      # observations
      ?observation a qb:Observation ;
        qb:dataSet <https://ld.stadt-zuerich.ch/statistics/dataset/WRT-RAUM-ZEIT-BIL-BTA> ;
        ?property ?value.

      # dimensions
      ?observation
        <https://ld.stadt-zuerich.ch/statistics/property/BIL> ?bil;
      ...
      ?bil skos:notation ?bilNotation .
      ?bta skos:notation ?btaNotation .
      ?raum skos:notation ?raumNotation .

      # filters
      ${typeof bil !== 'undefined' ? 'FILTER (?bilNotation IN (' + (bil.join ? bil.map(v => v.toCanonical()).join() : bil.toCanonical()) + '))' : ''}
