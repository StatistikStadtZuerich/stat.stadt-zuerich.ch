...
CONSTRUCT {
  <https://stat.stadt-zuerich.ch/dataset/STF-RAUM-ZEIT-BBA-ZON/slice> a qb:Slice ;
...
} WHERE {
  {
    GRAPH <https://linked.opendata.swiss/graph/zh/statistics> {
      # observations
      ?observation a qb:Observation ;
        qb:dataSet <https://ld.stadt-zuerich.ch/statistics/dataset/STF-RAUM-ZEIT-BBA-ZON> ;
        ?property ?value.

      # dimensions
      ?observation
        <https://ld.stadt-zuerich.ch/statistics/property/BBA> ?bba;
      ...
      # filters
      ${typeof bba !== 'undefined' ? 'FILTER (?bbaNotation IN (' + (bba.join ? bba.map(v => v.toCanonical()).join() : bba.toCanonical()) + '))' : ''}
