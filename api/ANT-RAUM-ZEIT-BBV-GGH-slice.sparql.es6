...
CONSTRUCT {
  <https://stat.stadt-zuerich.ch/dataset/ANT-RAUM-ZEIT-BBV-GGH/slice> a qb:Slice ;
...
} WHERE {
  {
    GRAPH <https://linked.opendata.swiss/graph/zh/statistics> {
      # observations
      ?observation a qb:Observation ;
        qb:dataSet <https://ld.stadt-zuerich.ch/statistics/dataset/ANT-RAUM-ZEIT-BBV-GGH> ;
        ?property ?value.

      # dimensions
      ?observation
        <https://ld.stadt-zuerich.ch/statistics/property/BBV> ?bbv;
      ...
      ?bbv skos:notation ?bbvNotation .
      ?ggh skos:notation ?gghNotation .

      # filters
      ${typeof bbv !== 'undefined' ? 'FILTER (?bbvNotation IN (' + (bbv.join ? bbv.map(v => v.toCanonical()).join() : bbv.toCanonical()) + '))' : ''}
