...
CONSTRUCT {
  <https://stat.stadt-zuerich.ch/dataset/MBL-RAUM-ZEIT-KNV-SEX-WAB/slice> a qb:Slice ;
...
} WHERE {
  {
    GRAPH <https://linked.opendata.swiss/graph/zh/statistics> {
      # observations
      ?observation a qb:Observation ;
        qb:dataSet <https://ld.stadt-zuerich.ch/statistics/dataset/MBL-RAUM-ZEIT-KNV-SEX-WAB> ;
        ?property ?value.

      # dimensions
      ?observation
        <https://ld.stadt-zuerich.ch/statistics/property/KNV> ?knv;
      ...
      ?knv skos:notation ?knvNotation .
      ?sex skos:notation ?sexNotation .
      ?wab skos:notation ?wabNotation .

      # filters
      ${typeof knv !== 'undefined' ? 'FILTER (?knvNotation IN (' + (knv.join ? knv.map(v => v.toCanonical()).join() : knv.toCanonical()) + '))' : ''}
