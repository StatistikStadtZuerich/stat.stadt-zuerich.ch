...
CONSTRUCT {
  <https://stat.stadt-zuerich.ch/dataset/SSA-RAUM-ZEIT-BTA-SAR-STS/slice> a qb:Slice ;
...
} WHERE {
  {
    GRAPH <https://linked.opendata.swiss/graph/zh/statistics> {
      # observations
      ?observation a qb:Observation ;
        qb:dataSet <https://ld.stadt-zuerich.ch/statistics/dataset/SSA-RAUM-ZEIT-BTA-SAR-STS> ;
        ?property ?value.
      ...
      ${typeof sts !== 'undefined' ? 'FILTER (?stsNotation IN (' + (sts.join ? sts.map(v => v.toCanonical()).join() : sts.toCanonical()) + '))' : ''}
