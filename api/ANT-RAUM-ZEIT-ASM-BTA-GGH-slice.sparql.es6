...
CONSTRUCT {
  <https://stat.stadt-zuerich.ch/dataset/ANT-RAUM-ZEIT-ASM-BTA-GGH/slice> a qb:Slice ;
...
} WHERE {
  {
    GRAPH <https://linked.opendata.swiss/graph/zh/statistics> {
      # observations
      ?observation a qb:Observation ;
        qb:dataSet <https://ld.stadt-zuerich.ch/statistics/dataset/ANT-RAUM-ZEIT-ASM-BTA-GGH> ;
        ?property ?value.
      ...
      # filters
      ${typeof asm !== 'undefined' ? 'FILTER (?asmNotation IN (' + (asm.join ? asm.map(v => v.toCanonical()).join() : asm.toCanonical()) + '))' : ''}
